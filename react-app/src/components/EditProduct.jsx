...
function EditProduct() {
  ...
  const [productoldimage2, setproductoldimage2] = useState('');
    ...
    axios.get(url)
      .then((res) => {
        if (res.data.product) {
          ...
          setproductoldimage(product.productimage)
          setproductoldimage2(product.productimage2)
        }
      })
      .catch((err) => {
        alert('server error')
  ...
  return (
    <div>
      ...
      <div className="p-3">
        ...
        />
        <img src={API_URL + '/' + productoldimage} width="100" height="50" /> <br></br>
        ...
        />
        <img src={API_URL + '/' + productoldimage2} width="100" height="50" /> <br></br>
        {/* <label> Product's Second Image</label>
        <input className='form-control' type='file'
          onChange={(event) => {
            setproductimage2(event.target.files[0])
